...
foreach ($path in $paths) {
    ...
    Try {
        ...
        $zoneIdentifierPath = "$path:Zone.Identifier"
        
        if (Test-Path $zoneIdentifierPath) {
            Write-Host $zoneIdentifierPath
            $urlLine = Get-Content $zoneIdentifierPath | Select-String -Pattern "HostURL="
            Write-Host $urlLine
        }
