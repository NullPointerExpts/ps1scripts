...
foreach($out in $output) {
    ...
    if ([System.Text.RegularExpressions.Regex]::IsMatch($line, $filter)) {
            ...
            if (Test-Path $path) {
                ...
                Write-Host "[Explorer] Detected deleted .exe: "$path -ForegroundColor Red
            }
        ...
        foreach($out in $output) {
            ...
            if ([System.Text.RegularExpressions.Regex]::IsMatch($line, $filter)) {
                    if (Test-Path $line) {
                        ...
                        Write-Host "[PCA] Detected deleted .exe: "$line -ForegroundColor Red
                    }   
